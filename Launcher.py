...
TOOLS = [
    ("PDFScraper.py", "--in_pdf", "--out_csv",
     "Static/Templates/Plant Guide 2025 Update.pdf", "Plants_NeedLinks", ".csv"),

    ("GetLinks.py", "--in_csv", "--out_csv",
     "Static/Outputs/Plants_NeedLinks.csv", "Plants_Linked", ".csv"),

    ("FillMissingData.py", "--in_csv", "--out_csv",
     "Static/Outputs/Plants_Linked.csv", "Plants_Linked_Filled", ".csv"),

    ("GeneratePDF.py", "--in_csv", "--out_pdf",
     "Static/Templates/Plants_Linked_Filled_Master.csv", "Plant_Guide_EXPORT", ".pdf"),

    ("Excelify2.py", "--in_csv", "--out_xlsx",
     "Static/Templates/Plants_Linked_Filled_Master.csv", "Plants_Linked_Filled_Review", ".xlsx"),
]
...
def refresh_out_labels():
    for _, lbl, stem, ext in out_widgets:
        lbl.configure(text=str(Path(out_dir_var.get()) / f"{pre_var.get()}{stem}{suf_var.get()}{ext}"))
...
for script, in_flag, out_flag, def_in, stem, ext in TOOLS:
    ...
    nice_name = LABEL_OVERRIDES.get(script, script.replace(".py", "").replace("_", " ").title())
    ctk.CTkLabel(title_row, text=nice_name, font=("Arial", 14, "bold")).pack(side="left")
    ctk.CTkButton(title_row, text="▶ Run", width=70,
    ...
    var = in_vars.setdefault((script, in_flag),
                             ctk.StringVar(value=str(BASE / def_in)))
    ctk.CTkEntry(in_row, textvariable=var, width=380).pack(side="left", padx=4)
    ctk.CTkButton(in_row, text="Browse…",
                  command=lambda v=var, f=in_flag: choose_input(v, f)
                  ).pack(side="left")
