...
# ─── Scripts Sheet: Code Block Placeholders ───────────────────────────────
code_sheet = wb.create_sheet("Scripts")
code_sheet.sheet_properties.tabColor = "C0C0C0"
code_sheet.column_dimensions["A"].width = 100
...
def draw_code_box(ws, start_row: int, label: str):
    ws[f"A{start_row}"] = f"📄 {label}"
...
for i, name in enumerate(script_descriptions, start=0):
    draw_code_box(code_sheet, i * 12 + 1, name)
