...
def safe_print(*objs, **kw):
    """
    Print without crashing on consoles that use legacy code-pages.
    Any glyph the encoding can’t handle is silently removed.
    """
    try:
        print(*objs, **kw)
...
def guess_common(lines: List[str], bot_idx: int) -> str:
    ...
    for ln in lines[bot_idx + 1 :]:
        lower = ln.lower()
        if any(
            tag in lower for tag in ["plant symbol", "description", "contributed by"]
        ):
            continue
        return ln
    return ""
...
def build_page_type_map(pdf_path: Path) -> Dict[int, str]:
    ...
    for i, (start_page, label) in enumerate(title_pages):
        end_page = (
            title_pages[i + 1][0] if i + 1 < len(title_pages) else total_pages + 1
        )
    ...
    with pdfplumber.open(PDF_PATH) as pdf:
        for idx, pg in enumerate(tqdm(pdf.pages, desc="Scanning PDF")):
            ...
            if bot_idx is None and len(lines) >= 2:
                pb, pc = clean(lines[0]), clean(lines[1])
                if BOT_LINE_RE.match(pb):
                    bot_idx, bot_name, com_name = 0, pb, pc
